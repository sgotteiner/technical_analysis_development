...
class FlagPatternBlock(BaseBlock):
    """
    Category 2: Chart Shape Patterns (Bull Flags)
    """
    def __init__(self, tf='1H', pivot_window=3, lookback=60):
        super().__init__(name=f"Bull Flags ({tf})", category="Shape", tf=tf)
    ...
    def evaluate(self, df_daily, df_1h) -> BlockResult:
        ...
        for i in range(self.lookback, n):
            ...
            if (h_slope < 0) and (l_slope < 0) and abs(h_slope - l_slope) / (abs(h_slope) + 1e-5) < 0.85:
                ...
                if df['Close'].iloc[i-1] <= proj_res and df['Close'].iloc[i] > proj_res:
                    ...
                    details[i] = {'type': 'Bull Flag', 'res': res_seg, 'sup': sup_seg}
